...
async function startHisoka() {
    ...
    hisoka.ev.on('messages.upsert', async chatUpdate => {
        ...
        require("./handler")(hisoka, m, chatUpdate, store)
        } catch (err) {
            console.log(err)
        }
    })
        ...
        try {
            ...
            for (let num of participants) {
                ...
                if (anu.action == 'add') {
                    ...
                    anu_user = num.split("@")[0]
        //time_wel = moment.tz("Asia/Jakarta").format("HH:mm")
        wel = `Halo @${anu_user} \nWelcome In ${metadata.subject}`
        buff = await getBuffer(
          `http://hadi-api.herokuapp.com/api/card/welcome?nama=${anu_user}&descriminator=${groupMembers.length}&memcount=${memeg}&gcname=${encodeURI(mdata.subject)}&pp=${pp_user}&bg=https://i.ibb.co/Jqn8mQ8/20211013-212323.jpg`
        )
but = [
